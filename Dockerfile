...
ARG TARGETARCH
ARG TARGETPLATFORM
RUN echo "BUILDING for ARCH: $TARGETARCH, PLATFORM: $TARGETPLATFORM"
...
# Python packages - flickr_download is installed as a dependency of flickrtoimmich
...
# Note: flickrtoimmich.download_wrapper also applies this patch at runtime via monkeypatching,
# but patching the source ensures the fix is active even when flickr_download is used directly.
...
COPY flickrtoimmich/ /build/flickrtoimmich/
